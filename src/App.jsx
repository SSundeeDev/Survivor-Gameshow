      ...
      <div style={{display: 'flex', flexDirection: 'column', zoom: textScale}}>
        {mode === 'host' && (
          <BuildMode
            ...
            subtitle="Build the answer key before the show starts. Seal when done."
            property={property}
            items={items}
      ...
      <div style={{
        ...
        <div style={{flex: 1, display: 'flex', alignItems: 'center', gap: 16}}>
          <div style={{
            ...
            <div className="tra-display" style={{fontSize: 14, letterSpacing: '0.15em'}}>{who}</div>
          </div>
          {extraHeader}
          <div className="tra-body" style={{fontSize: 12, color: 'rgba(168,197,232,0.7)'}}>{subtitle}</div>
        </div>
...
function ContestantBuildWrapper({property, items, activeContestant, setActiveContestant, p1, p2}) {
  ...
  const nameInput = (
    <div style={{display: 'flex', alignItems: 'center', gap: 8}}>
      <span className="tra-mono" style={{fontSize: 9, letterSpacing: '0.22em', color: 'rgba(168,197,232,0.5)'}}>NAME:</span>
      <input
        type="text"
        value={active.name}
        onChange={(e) => active.setName(e.target.value)}
        className="tra-body"
        style={{
  ...
  return (
    <div style={{display: 'flex', flexDirection: 'column'}}>
      ...
      <div style={{
        ...
        {[
          ...
          return (
            ...
            >
              {c.label.toUpperCase()}
              <span className="tra-mono" style={{fontSize: 9, color: 'rgba(168,197,232,0.6)', letterSpacing: '0.15em'}}>
                {c.count} ITEMS
              </span>
              {c.locked && (
      ...
      <BuildMode
        ...
        subtitle="Think out loud, talk through every pick. No timer. Seal when you're ready."
        property={property}
        items={items}
...
// ============================================================
function RevealMode({hostName, items, treyPackage, p1, p2, revealedItems, setRevealedItems, soundEnabled}) {
  const [activeCat, setActiveCat] = useState('all');
  const [filterMode, setFilterMode] = useState('all'); // 'all' | 'unrevealed' | 'disagree' | 'mismatch'
  const [search, setSearch] = useState('');
  const [lastRevealedId, setLastRevealedId] = useState(null);
  const fmt = (n) => '$' + n.toLocaleString();

  // Score = 100 - (# mismatches among REVEALED items only)
  const calcScore = (playerPackage) => {
    let score = 100;
    revealedItems.forEach(itemId => {
  ...
  // Track previous scores to detect drops and trigger animations
  ...
  const filterPipeline = useMemo(() => {
    ...
    if (filterMode === 'unrevealed') {
      ...
      list = list.filter(i => {
        if (!revealedItems.has(i.id)) return false;
    ...
    if (search) list = list.filter(i => itemMatchesSearch(i, search));
    return list;
  }, [activeCat, filterMode, items, revealedItems, p1.package, p2.package, treyPackage, search]);
  ...
  const revealItem = (id) => {
    ...
    if (wasRevealed) {
      next.delete(id);
      if (lastRevealedId === id) setLastRevealedId(null);
      playSfx('click', soundEnabled);
    } else {
      next.add(id);
      setLastRevealedId(id);
      // Decide which sound: any contestant mismatch = mismatch sound, otherwise match
      const treyIn = treyPackage.has(id);
      const p1Match = p1.package.has(id) === treyIn;
      const p2Match = p2.package.has(id) === treyIn;
      const anyMismatch = !p1Match || !p2Match;
  ...
  const allRevealed = revealedItems.size === items.length;
  const winner = allRevealed
    ? (p1Score === p2Score ? 'TIE' : p1Score > p2Score ? p1.name : p2.name)
    : null;

  // Play winner/tie sound once when reveal completes
  const winnerPlayedRef = useRef(false);
  useEffect(() => {
    if (allRevealed && !winnerPlayedRef.current) {
      winnerPlayedRef.current = true;
      // Slight delay so it lands after the last reveal sound
      setTimeout(() => {
        playSfx(winner === 'TIE' ? 'tie' : 'winner', soundEnabled);
      }, 350);
    } else if (!allRevealed) {
  ...
  // Counts per filter mode for badges
  ...
  const mismatchCount = Array.from(revealedItems).filter(id => {
    const treyIn = treyPackage.has(id);
    ...
    return (
      <div style={{flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 40}}>
        <div style={{textAlign: 'center', maxWidth: 500}}>
          <div className="tra-display" style={{fontSize: 28, letterSpacing: '0.04em', color: OFF_WHITE, marginBottom: 12}}>
            NO PACKAGES TO REVEAL YET
          </div>
  ...
  return (
    <div style={{display: 'flex', flexDirection: 'column'}}>
          ...
          {filterTabs.map(f => {
            ...
            return (
              <button
                ...
                disabled={f.count === 0 && !active}
                className="tra-display"
                style={{
                  padding: '5px 11px',
                  fontSize: 10,
                  letterSpacing: '0.15em',
                  border: `1px solid ${active ? f.color : dim ? 'rgba(168,197,232,0.1)' : 'rgba(168,197,232,0.25)'}`,
                  background: active ? f.color : 'transparent',
                  color: active ? NAVY : dim ? 'rgba(168,197,232,0.3)' : f.color,
                  cursor: dim ? 'not-allowed' : 'pointer',
                  display: 'flex',
                  alignItems: 'center',
                  gap: 6,
                ...
                title={
                  f.id === 'disagree' ? 'Items where the two contestants made different calls — the high-stakes reveals' :
                  f.id === 'mismatch' ? 'Revealed items where someone got it wrong' :
                  f.id === 'unrevealed' ? 'Items not yet shown' : 'Show all items'
                }
              >
                {f.label}
                <span className="tra-mono" style={{
      ...
      <div style={{
        ...
        <span className="tra-mono" style={{fontSize: 9, letterSpacing: '0.22em', color: 'rgba(168,197,232,0.5)'}}>CATEGORY:</span>
        <div style={{flex: 1, minWidth: 0}}>
        ...
        <div className="tra-mono" style={{fontSize: 10, letterSpacing: '0.18em', color: 'rgba(168,197,232,0.6)'}}>
          REVEALED: <span style={{color: GOLD}}>{revealedItems.size} / {items.length}</span>
        </div>
        <button onClick={resetReveals} className="tra-mono" style={{padding: '5px 10px', background: 'transparent', border: '1px solid rgba(168,197,232,0.3)', color: 'rgba(168,197,232,0.7)', fontSize: 9, letterSpacing: '0.15em', cursor: 'pointer'}}>
          RESET REVEALS
        </button>
        <button onClick={revealAll} className="tra-mono" style={{padding: '5px 10px', background: 'rgba(201,182,131,0.15)', border: `1px solid ${GOLD}`, color: GOLD, fontSize: 9, letterSpacing: '0.15em', cursor: 'pointer'}}>
          REVEAL ALL
        </button>
      </div>
        ...
        {filterPipeline.length === 0 ? (
          ...
          <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 10}}>
            {filterPipeline.map(item => {
              ...
              const isLast = lastRevealedId === item.id;
              const isContested = p1In !== p2In; // contestants disagreed with each other

              // Card highlight: contested = subtle gold pulse hint when unrevealed
              const cardBorderColor = revealed
                ? 'rgba(201,182,131,0.4)'
                : isContested
                  ? 'rgba(201,182,131,0.35)'
                  : 'rgba(168,197,232,0.15)';

              return (
                <div
                  key={item.id}
                  onClick={() => revealItem(item.id)}
                  className={isLast && revealed ? 'reveal-card-anim' : ''}
                  style={{
                    padding: '10px 12px',
                    background: revealed ? 'rgba(10,21,41,0.8)' : 'rgba(10,21,41,0.4)',
                    border: `1px solid ${cardBorderColor}`,
                    borderLeft: `3px solid ${revealed ? GOLD : isContested ? 'rgba(201,182,131,0.55)' : 'rgba(168,197,232,0.15)'}`,
                    cursor: 'pointer',
                    transition: 'all 0.15s',
                    userSelect: 'none',
                    position: 'relative',
                ...
                >
                  {/* Contested marker on unrevealed cards */}
                  {!revealed && isContested && (
                    <div
                      className="tra-mono"
                      style={{
                        position: 'absolute', top: 8, right: 8,
                        fontSize: 7, padding: '2px 6px',
                        background: 'rgba(201,182,131,0.15)',
                        border: '1px solid rgba(201,182,131,0.4)',
                        color: GOLD,
                  ...
                  <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8}}>
                    <div style={{flex: 1, paddingRight: 8}}>
                      ...
                      <div className="tra-body" style={{fontSize: 12, fontWeight: 600, color: OFF_WHITE, lineHeight: 1.25, paddingRight: !revealed && isContested ? 70 : 0}}>{item.name}</div>
                    </div>
                  ...
                  {revealed ? (
                    <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 4, marginTop: 8}}>
                      <CallBadge label={hostName.toUpperCase()} on={treyIn} accent={GOLD} animate={isLast} />
                      <CallBadge label={p1.name.toUpperCase()} on={p1In} match={p1Match} accent="#7FB5E8" animate={isLast} />
                      <CallBadge label={p2.name.toUpperCase()} on={p2In} match={p2Match} accent="#E8A87F" animate={isLast} />
                    </div>
                  ) : (
                    <div className="tra-mono" style={{fontSize: 10, letterSpacing: '0.18em', color: 'rgba(168,197,232,0.4)', textAlign: 'center', padding: '6px 0'}}>
                      CLICK TO REVEAL
                    </div>
                  )}
                </div>
...
function ScorePanel({name, score, color, isWinner, isTie, flash}) {
  return (
    <div
      ...
      style={{
        ...
        borderLeft: `5px solid ${isWinner ? GREEN : color}`,
        display: 'flex',
    ...
    >
      ...
      <div style={{position: 'relative', zIndex: 1}}>
        <div className="tra-mono" style={{fontSize: 10, letterSpacing: '0.32em', color: 'rgba(168,197,232,0.55)'}}>
          ◆ CONTESTANT
        </div>
        <div className="tra-display" style={{fontSize: 30, letterSpacing: '0.06em', color: OFF_WHITE, marginTop: 4, lineHeight: 1, textShadow: '0 2px 8px rgba(0,0,0,0.6)'}}>
          {name.toUpperCase()}
        </div>
      </div>
      <div style={{textAlign: 'right', position: 'relative', zIndex: 1}}>
        <div className="tra-mono" style={{fontSize: 10, letterSpacing: '0.32em', color: 'rgba(168,197,232,0.55)'}}>SCORE</div>
        <div className="tra-display" style={{
          fontSize: 64,
          ...
          lineHeight: 1,
          marginTop: 2,
          textShadow: `0 0 30px ${color}50, 0 2px 8px rgba(0,0,0,0.5)`,
          fontWeight: 700,
        }}>
      ...
      {isWinner && !isTie && (
        <div className="tra-display badge-anim" style={{
          ...
          fontSize: 11, color: GREEN, letterSpacing: '0.22em',
          padding: '3px 10px',
          background: 'rgba(61,169,122,0.18)',
          border: `1px solid ${GREEN}`,
          textShadow: `0 0 12px rgba(61,169,122,0.6)`,
        }}>🏆 WINNER</div>
      )}
    </div>
  );
