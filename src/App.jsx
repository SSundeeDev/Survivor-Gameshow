...
function RevealMode({hostName, items, treyPackage, p1, p2, revealedItems, setRevealedItems, soundEnabled}) {
  ...
  // Track scroll so the scoreboard can hide at the top and shrink when stuck
  ...
  useEffect(() => {
    ...
    // Wait until the user has scrolled well past the full-size scoreboard
    // (header ~240 + full scoreboard ~150 = ~390)
    const THRESHOLD = 420;
  ...
  return (
    <div style={{display: 'flex', flexDirection: 'column'}}>
      {/* Compact sticky scoreboard — fixed overlay, no layout shift */}
      {scoreboardStuck && (
        <div style={{
          background: `linear-gradient(180deg, ${NAVY_DEEP} 0%, rgba(5,11,24,0.98) 100%)`,
          padding: '8px 24px',
          borderBottom: '1px solid rgba(201,182,131,0.25)',
          display: 'grid',
      ...
      {/* Full-size scoreboard — only at the top of the page (not sticky) */}
      <div style={{
        background: `linear-gradient(180deg, ${NAVY_DEEP} 0%, rgba(5,11,24,0.98) 100%)`,
        padding: '16px 24px',
        borderBottom: '1px solid rgba(201,182,131,0.2)',
        display: 'grid',
        gridTemplateColumns: '1fr 1fr',
        gap: 20,
        flexShrink: 0,
